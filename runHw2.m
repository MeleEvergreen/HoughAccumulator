...
signAcademicHonestyPolicy('Shantanu Gupta', 'University of Wisconsin-Madison');
...
canny_threshold = [0.1, 0.1, 0.25];
for i = 1:length(img_list)
    img = imread([img_list{i} '.png']);
    edge_img = edge(img, 'canny', canny_threshold(i));
...
for i = 1:length(img_list)
    ...
    hough_accumulator = 255 * (hough_accumulator / max(hough_accumulator(:)));
